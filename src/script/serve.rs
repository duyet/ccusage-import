...
pub async fn run(args: ServeArgs) -> anyhow::Result<()> {
    ...
    let bind = args
        .bind
        .or_else(|| cfg.telemetry.bind.clone())
        .or_else(|| std::env::var("SUMMA_TELEMETRY_BIND").ok())
        .filter(|s| !s.is_empty())
    ...
    let token = cfg
        ...
        .token
        .clone()
        .or_else(|| std::env::var("SUMMA_TELEMETRY_TOKEN").ok())
        .unwrap_or_default();
    let duckdb_path = Config::resolve_duckdb_path(cfg.importer.duckdb_path.as_deref());
    let clickhouse_enabled = !cfg.clickhouse.host.is_empty();
    ...
    eprintln!("summa serve listening on http://{bind}");
    eprintln!("  duckdb={duckdb_path}");
    eprintln!(
        "  clickhouse={}",
        if clickhouse_enabled { "on" } else { "off" }
    );
