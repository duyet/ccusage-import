...
/// The 29-column schema for ccusage_events, in authoritative order.
...
const CH_ENGINE_SUFFIX: &str = "ENGINE = ReplacingMergeTree(updated_at) PARTITION BY toYYYYMM(date) ORDER BY (source, machine_name, record_type, date, model_name, record_key)";
...
mod tests {
    ...
    #[test]
    fn column_count_is_29() {
        assert_eq!(EVENTS_COLUMNS.len(), 29);
    }
